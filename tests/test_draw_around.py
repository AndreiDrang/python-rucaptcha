...
from python_rucaptcha.core.enums import SaveFormatsEnm, DrawAroundCaptchaEnm
...
class TestDrawAroundCaptcha(BaseTest):
    ...
    def test_methods_exists(self):
        assert "captcha_handler" in DrawAroundCaptcha.__dict__.keys()
        assert "aio_captcha_handler" in DrawAroundCaptcha.__dict__.keys()
    ...
    def test_args(self, save_format: str, img_clearing: bool):
        instance = DrawAroundCaptcha(
            rucaptcha_key=self.RUCAPTCHA_KEY,
            img_clearing=img_clearing,
            save_format=save_format,
        )
        assert instance.create_task_payload["clientKey"] == self.RUCAPTCHA_KEY
        assert instance.create_task_payload["task"]["type"] == DrawAroundCaptchaEnm.DrawAroundTask
        assert instance.save_format == save_format
        assert instance.img_clearing == img_clearing

    def test_kwargs(self):
        instance = DrawAroundCaptcha(rucaptcha_key=self.RUCAPTCHA_KEY, **self.kwargs_params)
    ...
    def test_basic_file(self, save_format):
        instance = DrawAroundCaptcha(rucaptcha_key=self.RUCAPTCHA_KEY, save_format=save_format)
